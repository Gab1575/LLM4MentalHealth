...
def Resting(publisher, stop_event, initial_state, logger):
    """Runs the resting idle state in a loop until stop_event is set.
    """
    logger.info("Starting resting routine...")
    ...
    while not stop_event.is_set():
        ...
        # S4 flips between 15 and -15 every 10 seconds
        position = int(elapsed // 10) % 2
        routine_msg.servo_angles[3] = 15 if position == 0 else -15
    ...
    logger.info("Resting routine stopped. Restoring original GUI state...")
